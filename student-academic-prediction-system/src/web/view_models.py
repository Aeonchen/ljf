...
def build_risk_distribution(risk_labels):
    counts = risk_labels.value_counts().reindex(['高风险', '中风险', '低风险'], fill_value=0)
    return {
...
def build_feature_importance_frame(model, top_n=10):
...
def build_prediction_probability_rows(classes, probabilities):
    ...
    return pd.DataFrame({
        ...
        'color': ['#EF4444', '#F59E0B', '#10B981'][:len(probability_values)],
    })
...
def build_prediction_display(prediction):
    ...
    return {
        'css_class': 'risk-low',
        'icon': '🟢',
        'message': '低风险 - 表现良好，继续保持',
    }


def build_prediction_recommendations(prediction):
    if prediction == '高风险':
        return [
            '立即安排一对一辅导',
            '联系家长沟通情况',
...
def filter_student_results(results_df, student_id=None, min_grade=None, max_grade=None, grade_column='GRADE'):
    if student_id is not None:
        return results_df[results_df['STUDENT ID'] == student_id]
    if min_grade is not None and max_grade is not None:
        return results_df[
            (results_df[grade_column] >= min_grade) &
            (results_df[grade_column] <= max_grade)
        ]
    return results_df
